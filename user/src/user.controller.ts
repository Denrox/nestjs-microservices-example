...
export class UserController {
  ...
  public async searchUserByCredentials(
    ...
    if (searchParams.email && searchParams.password) {
      const user = await this.userService.searchUser({email: searchParams.email});
  ...
  public async confirmUser(
    ...
    if (confirmParams) {
      ...
      if (userLink && userLink[0]) {
        ...
        await this.userService.updateUserLinkById(
          userLink[0].id,
          {
            is_used: true
          }
  ...
  public async createUser(
    ...
    if (userParams) {
      const usersWithEmail = await this.userService.searchUser({email: userParams.email});
